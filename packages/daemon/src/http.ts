...
export function startHttpApi(options: HttpApiOptions): Server<undefined> {
  const { secret, actor, repoId, inbox, transport, openRuntime } = options;
  ...
  async function handle(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/session-event") {
        const event = SessionEventSchema.parse(await request.json());
        const runtime = await openRuntime();
        try {
          await runtime.core.appendSessionEvent(event);
        } finally {
          runtime.close();
        }
        return json({ ok: true });
